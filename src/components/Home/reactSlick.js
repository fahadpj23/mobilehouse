      ...
      <div className="px-0 md:px-3  w-full md:w-11/12  ">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8">
          {props.category && props.category.map((item, key) => {
              ...
              return(
                <Link className="  h-32 md:h-48 px-2 py-2 " to={{pathname: "/ProductList",search: "?" + new URLSearchParams({type:'category',PageNo:1,category: item.id,sort:"newestfirst"}).toString()}}>
