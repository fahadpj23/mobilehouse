            ...
            <div className="grid grid-cols-3 w-full gap-x-3  ">
            {
                props.Ads && props.Ads.detail?.map((item,key)=>{
                    ...
                    <div className="p-3 space-y-5  shadow-lg border border-gray-200   rounded">
                        <Link  to={{pathname: "/ProductList",search: "?" + new URLSearchParams({type:'Brand',PageNo:1,Brand:item.Brand,sort:"newestfirst"}).toString(),}}   className="w-full"><img src={`${process.env.REACT_APP_MOBILE_HOUSEIMAGE}/${item.image}`} alt="ads image" className="object-fill w-full h-64 sm:h-40 md:h-64 overflow-hidden rounded" /></Link> 
