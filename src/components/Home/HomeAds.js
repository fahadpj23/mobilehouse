        ...
        <div className="w-full">
            <div className="grid grid-cols-3 w-full gap-x-3 ">
            {
                props.Ads && props.Ads.detail.map((item,key)=>{
                    return(
                    <Link  to={{pathname: "/ProductList",search: "?" + new URLSearchParams({Brand:item.Brand}).toString()}}   className=""><img src={`http://127.0.0.1:9000/images/${item.image}`} alt="ads image" className="object-fill h-72 overflow-hidden" /></Link> 
                    )
